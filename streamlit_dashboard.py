...
if ELECTRICITY_MAPS_AVAILABLE:
    with st.sidebar.expander("🌍 Electricity Maps - Day-Ahead Prices", expanded=False):
        ...
        # Try to get key from secrets first, then session state
        default_key = st.secrets.get("em_key", "")
        if default_key == "PASTE_YOUR_ELECTRICITY_MAPS_API_KEY_HERE":
            default_key = ""
        
        em_key = st.text_input(
            "Electricity Maps API Key",
            type="password",
            value=st.session_state.get("em_key") or default_key,
            help="Zet je key in .streamlit/secrets.toml onder 'em_key' (aanbevolen) of plak hier tijdelijk."
        )
        
        if em_key:
            st.session_state.em_key = em_key
        
        # Show clear instruction if no valid key
        if not em_key or em_key == "PASTE_YOUR_ELECTRICITY_MAPS_API_KEY_HERE":
            st.warning("⚠️ Geen geldige Electricity Maps API key gevonden.\n\n"
                       "1. Open `.streamlit/secrets.toml`\n"
                       "2. Vervang `PASTE_YOUR_ELECTRICITY_MAPS_API_KEY_HERE` door je echte key\n"
                       "3. Herstart Streamlit")
        
        if st.button("📥 Fetch Prices", type="secondary"):
            if not em_key or em_key == "PASTE_YOUR_ELECTRICITY_MAPS_API_KEY_HERE":
                st.error("Please enter your Electricity Maps API key in the field above or in secrets.toml")
            else:
                try:
